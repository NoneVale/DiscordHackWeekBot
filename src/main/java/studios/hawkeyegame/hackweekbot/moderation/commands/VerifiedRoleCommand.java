...
public class VerifiedRoleCommand implements CommandExecutor {
    @Override
            ...
            switch (args.length) {
                        ...
                        if (args[0].equalsIgnoreCase("set")) {
                            if (serverModel.getUnverifiedRole().equals(role.getIdAsString())) {
                                EmbedBuilder roleAlreadySet = new EmbedBuilder()
                                        .setDescription("That role has already is already the verified role.")
                                        .setColor(serverModel.getColor())
                                        .setFooter(serverModel.getFooter())
                                        .setTimestamp(Instant.now());
                            ...
                            serverModel.setVerifiedRole(role.getIdAsString());
                            EmbedBuilder roleAdded = new EmbedBuilder()
                                    .setDescription(role.getMentionTag() + " has been set to the verified role.")
                                    .setColor(serverModel.getColor())
                                    .setFooter(serverModel.getFooter())
                                    .setTimestamp(Instant.now());
                ...
                default:
                    EmbedBuilder cmdInfo = new EmbedBuilder()
                            .setDescription(serverModel.getCommandPrefix() + "verifiedrole remove \n"
                                    + serverModel.getCommandPrefix() + "verifiedrole set [role] \n")
                            .setColor(serverModel.getColor())
                            .setFooter(serverModel.getFooter())
                            .setTimestamp(Instant.now());
